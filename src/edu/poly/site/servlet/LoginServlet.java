...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (username == null) {
			PageInfo.preparedAndForwardSite(request, response, PageType.SITE_LOGIN_PAGE);
			return;
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (user != null  && user.getPassword().equals(form.getPassword())) {
				...
				{
					response.sendRedirect("http://localhost:8080/Poly.Asg/Admin/UserManagement");
				}
				else
				{
	...
	private boolean checkRole(User userModal) {
		if (userModal.getAdmin()) {
			return true;
		}
		return false;
	}
